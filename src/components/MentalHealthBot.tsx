...
import { MessageSquare, X, Send, Brain, Smile, Frown, Meh } from 'lucide-react';
  ...
  return (
    <>
      ...
      <motion.button
        initial={{ x: -100 }}
        animate={{ x: 0 }}
        className={`fixed left-6 bottom-6 z-50 p-4 rounded-full shadow-lg transition-colors ${
          isOpen ? 'bg-slate-700 hover:bg-slate-800' : 'bg-teal-600 hover:bg-teal-700'
        }`}
        onClick={toggleChat}
      >
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed left-6 bottom-24 z-50 w-[380px] bg-white rounded-2xl shadow-xl"
          >
            {/* Header */}
            <div className="flex items-center justify-between p-4 border-b border-slate-100">
              <div className="flex items-center space-x-2">
                <Brain className="w-6 h-6 text-teal-600" />
                <h3 className="font-semibold text-slate-800">Mental Wellness Assistant</h3>
              </div>
              <button
                onClick={toggleChat}
